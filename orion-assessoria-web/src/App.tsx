...
import { LoginAssessoriaView } from './auth/LoginAssessoriaView';
...
import type { OrionAssessoriaPublicInfo, OrionAssessoriaSessao, Usuario } from './types';
import { formatMatricula } from './utils/formatMatricula';
import { listaMenuOutrosSistemas } from './utils/sistemaDestinosMenu';
import { usuarioPodeAcessarOrionAssessoria } from './utils/sistemaAccess';

const DOC_TITLE = 'Órion Assessoria';
const accent = '#2dd4bf';
...
export default function App() {
  const [publicInfo, setPublicInfo] = useState<OrionAssessoriaPublicInfo | null>(null);
  const [publicErr, setPublicErr] = useState<string | null>(null);
  const [sessaoModulo, setSessaoModulo] = useState<OrionAssessoriaSessao | null>(null);
  ...
  const carregarSessao = useCallback(async () => {
    ...
    try {
      ...
      if (!usuarioPodeAcessarOrionAssessoria(me)) {
        removerTokenSession();
        removerAcessoIdSession();
        setCurrentUser(null);
        setBootstrapError(
          'Seu perfil não tem permissão para o Órion Assessoria. Um administrador deve incluir «Órion Assessoria» nos sistemas permitidos do seu usuário (Órion SAD → Cadastrar usuários).',
        );
        setLoading(false);
        return;
      }
  ...
  function handleLoginSuccess(usuario: Usuario) {
    if (!usuarioPodeAcessarOrionAssessoria(usuario)) {
      removerTokenSession();
      removerAcessoIdSession();
      setBootstrapError(
        'Seu perfil não tem permissão para o Órion Assessoria. Solicite ao administrador a inclusão do módulo nos sistemas permitidos do seu cadastro no SAD.',
      );
      return;
    }
    setBootstrapError(null);
  ...
  if (!currentUser) {
    return (
      <div className="app-assessoria app-assessoria--auth">
        {bootstrapError ? (
          <Box sx={{ px: 2, pt: 2, maxWidth: 560, mx: 'auto', flexShrink: 0 }}>
            <Alert severity="warning">{bootstrapError}</Alert>
          </Box>
        ) : null}
        <div className="app-assessoria__auth-shell">
          <LoginAssessoriaView onSuccess={handleLoginSuccess} />
        </div>
  ...
  return (
    <div className="app-assessoria">
        ...
        >
          <Stack direction="row" alignItems="center" spacing={2} sx={{ flex: 1, minWidth: 0 }}>
            <Box
              ...
              aria-hidden
            >
              <AssignmentInd sx={{ fontSize: 28, color: '#f0fdfa' }} />
            </Box>
            <Divider
            ...
            <Box sx={{ minWidth: 0 }}>
              <Stack direction="row" alignItems="center" gap={1} flexWrap="wrap" sx={{ mb: 0.25 }}>
                ...
                <Chip
                  icon={<AssignmentInd sx={{ fontSize: '16px !important', ml: '4px !important' }} />}
                  label="Assessoria"
                  size="small"
                  sx={{
                    height: 24,
          ...
          <Stack direction="row" alignItems="center" spacing={1.75} sx={{ flexShrink: 0 }}>
            ...
            >
              <IconButton
                ...
                aria-controls={avatarMenuAnchor ? 'user-menu-assessoria' : undefined}
                aria-haspopup="true"
      ...
      <main className="app-assessoria__main">
        {publicErr ? (
        ...
        >
          <Stack direction="row" alignItems="flex-start" spacing={2}>
            ...
            <Box>
              <Typography variant="h6" sx={{ fontWeight: 700, color: '#f0fdfa', mb: 1 }}>
                Módulo em desenvolvimento
              </Typography>
              <Typography variant="body2" sx={{ color: 'var(--text-secondary)', lineHeight: 1.6, mb: 1 }}>
                Em desenvolvimento — funcionalidades de assessoria serão adicionadas aqui.
              </Typography>
              {sessaoModulo ? (
