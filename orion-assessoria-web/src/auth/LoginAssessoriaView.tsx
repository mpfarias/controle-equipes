...
interface LoginAssessoriaViewProps {
  onSuccess: (usuario: Usuario) => void;
}

export function LoginAssessoriaView({ onSuccess }: LoginAssessoriaViewProps) {
    ...
    >
      <Box
        ...
        sx={{
          ...
          background: `linear-gradient(105deg, ${alpha('#0f766e', 0.92)} 0%, ${alpha('#0f172a', 0.96)} 55%, ${alpha('#14b8a6', 0.22)} 100%)`,
          boxShadow: `0 4px 24px ${alpha('#000', 0.35)}`,
        }}
      >
        <Stack direction="row" alignItems="center" spacing={1.5} flexWrap="wrap" sx={{ gap: 1.5 }}>
          <AssignmentInd sx={{ fontSize: 32, color: accent }} aria-hidden />
          <Box>
            <Typography
              variant="overline"
              sx={{
                color: alpha('#f0fdfa', 0.5),
            ...
            <Typography
              component="h1"
              variant="h5"
              sx={{ fontWeight: 800, color: '#f0fdfa', letterSpacing: '-0.02em', mt: 0.5 }}
            >
              Órion Assessoria
            </Typography>
            <Typography variant="body2" sx={{ color: alpha('#f0fdfa', 0.65), mt: 0.5, maxWidth: 640 }}>
