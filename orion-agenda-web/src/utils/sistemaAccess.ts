...
/** Administrador, nível ADMINISTRADOR ou `ORION_AGENDA` em sistemas permitidos (cadastro SAD). */
export function usuarioPodeAcessarOrionAgenda(usuario: Usuario | null): boolean {
  if (!usuario) return false;
  if (usuario.isAdmin === true) return true;
  const nomeNivel = usuario.nivel?.nome?.trim().toUpperCase();
  if (nomeNivel === 'ADMINISTRADOR') return true;
  return (usuario.sistemasPermitidos ?? []).some((s) => {
    const id = String(s).trim().toUpperCase();
    return id === 'ORION_AGENDA' || id === 'ORION_ASSESSORIA';
  });
}
