/**
 * Alinhado ao ecossistema Órion (mesmas chaves que SAD/Suporte quando na mesma origem).
 * Inclui legado deste app para migração de sessão.
 */
...
const LEGACY_TOKEN_KEYS = ['orion-agenda-web:token', 'orion-assessoria-web:token'] as const;
const LEGACY_ACESSO_KEYS = ['orion-agenda-web:acessoId', 'orion-assessoria-web:acessoId'] as const;
...
export function gravarTokenSession(token: string): void {
  ...
  for (const k of LEGACY_TOKEN_KEYS) sessionStorage.removeItem(k);
}
