...
import { LoginJuridicoView } from './auth/LoginJuridicoView';
...
import { usuarioPodeAcessarOrionJuridico } from './utils/sistemaAccess';
...
export default function App() {
  ...
  const carregarSessao = useCallback(async () => {
    ...
    try {
      ...
      if (!usuarioPodeAcessarOrionJuridico(me)) {
        removerTokenSession();
        removerAcessoIdSession();
        setCurrentUser(null);
        setBootstrapError(
          'Seu perfil não tem permissão para o Órion Jurídico. Um administrador deve incluir «Órion Jurídico» nos sistemas permitidos do seu usuário (Órion SAD → Cadastrar usuários).',
        );
        setLoading(false);
        return;
      }
  ...
  function handleLoginSuccess(usuario: Usuario) {
    if (!usuarioPodeAcessarOrionJuridico(usuario)) {
      removerTokenSession();
      removerAcessoIdSession();
      setBootstrapError(
        'Seu perfil não tem permissão para o Órion Jurídico. Solicite ao administrador a inclusão do módulo nos sistemas permitidos do seu cadastro no SAD.',
      );
      return;
    }
    setBootstrapError(null);
  ...
  if (!currentUser) {
    return (
      <div className="app-juridico app-juridico--auth">
        {bootstrapError ? (
          <Box sx={{ px: 2, pt: 2, maxWidth: 560, mx: 'auto', flexShrink: 0 }}>
            <Alert severity="warning">{bootstrapError}</Alert>
          </Box>
        ) : null}
        <div className="app-juridico__auth-shell">
          <LoginJuridicoView onSuccess={handleLoginSuccess} />
        </div>
  ...
  return (
    <div className="app-juridico">
          ...
          <Stack direction="row" alignItems="center" spacing={2} sx={{ flex: 1, minWidth: 0 }}>
            <Box
              sx={{
                ...
                height: { xs: 46, sm: 54 },
                borderRadius: 2.5,
                background: `linear-gradient(145deg, ${alpha(accent, 0.5)} 0%, ${alpha('#6d28d9', 0.92)} 50%, ${alpha('#1e1b4b', 1)} 100%)`,
              ...
              aria-hidden
            >
              <Gavel sx={{ fontSize: 28, color: '#f5f3ff' }} />
            </Box>
            <Divider
            ...
            <Box sx={{ minWidth: 0 }}>
              <Stack direction="row" alignItems="center" gap={1} flexWrap="wrap" sx={{ mb: 0.25 }}>
                ...
                <Chip
                  icon={<Gavel sx={{ fontSize: '16px !important', ml: '4px !important' }} />}
                  label="Jurídico"
                  size="small"
                  sx={{
                    height: 24,
          ...
          <Stack direction="row" alignItems="center" spacing={1.75} sx={{ flexShrink: 0 }}>
            ...
            <Box
              sx={{
                p: '3px',
                borderRadius: '50%',
                background: `linear-gradient(135deg, ${alpha(accent, 0.55)}, ${alpha('#1e1b4b', 0.35)})`,
              }}
            >
              <IconButton
                onClick={(e) => setAvatarMenuAnchor(e.currentTarget)}
                aria-label="Menu do usuário"
                aria-controls={avatarMenuAnchor ? 'user-menu-juridico' : undefined}
                aria-haspopup="true"
      ...
      <main className="app-juridico__main">
        {publicErr ? (
        ...
        >
          <Stack direction="row" alignItems="flex-start" spacing={2}>
            ...
            <Box>
              <Typography variant="h6" sx={{ fontWeight: 700, color: '#f5f3ff', mb: 1 }}>
                Módulo em desenvolvimento
              </Typography>
              <Typography variant="body2" sx={{ color: 'var(--text-secondary)', lineHeight: 1.6, mb: 1 }}>
                Em desenvolvimento — funcionalidades jurídicas serão adicionadas aqui.
              </Typography>
              {sessaoModulo ? (
