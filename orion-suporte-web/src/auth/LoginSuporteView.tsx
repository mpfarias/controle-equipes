...
import { CallSplit, Visibility, VisibilityOff, SupportAgent } from '@mui/icons-material';
import { api } from '../api';
...
interface LoginSuporteViewProps {
  onSuccess: (usuario: Usuario) => void;
}

export function LoginSuporteView({ onSuccess }: LoginSuporteViewProps) {
  ...
  const fieldSx = {
    ...
    '& .MuiInputLabel-root': { color: alpha('#ecfdf5', 0.65) },
    '& .MuiOutlinedInput-input': { color: '#ecfdf5' },
  } as const;
    ...
    <Box sx={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      <Box
        ...
        sx={{
          ...
          background: `linear-gradient(90deg, ${alpha('#042f2e', 0.95)} 0%, ${alpha('#0f172a', 0.92)} 100%)`,
      ...
      >
        <Stack direction="row" alignItems="center" spacing={1.5} flexWrap="wrap" sx={{ gap: 1.5 }}>
          ...
          <Box>
            ...
            <Typography variant="body2" sx={{ color: alpha('#ecfdf5', 0.62), mt: 0.5, maxWidth: 640 }}>
              <strong>Gestão de chamados técnicos.</strong> Isto não é o Órion SAD (efetivo, escalas, afastamentos).
              Você precisa <strong>entrar de novo aqui</strong> — a sessão do SAD, em outra aba, continua só lá.
            </Typography>
          </Box>
        </Stack>
      </Box>
      ...
      >
        ...
        >
          <Stack spacing={2} alignItems="center" sx={{ mb: 2.5 }}>
            <SupportAgent sx={{ fontSize: 44, color: accent }} aria-hidden />
            <Typography variant="subtitle1" sx={{ fontWeight: 700, color: accent, textAlign: 'center' }}>
              Acesso ao módulo Suporte
            </Typography>
          </Stack>
          ...
          >
            <Typography variant="body2" component="span" sx={{ display: 'block', lineHeight: 1.55 }}>
              ...
              abre <strong>somente</strong> o painel de chamados — é outro sistema no navegador.
            </Typography>
          </Alert>
          ...
          <Box component="form" onSubmit={(e) => void handleSubmit(e)} noValidate>
            ...
            <Stack spacing={2}>
              ...
              >
                {loading ? 'Entrando no Suporte…' : 'Entrar no Órion Suporte'}
              </Button>
            </Stack>
          </Box>
        </Paper>
