...
export function usuarioPodeAcessarOrionOperacoes(usuario: Usuario | null): boolean {
  if (!usuario) return false;
  return (usuario.sistemasPermitidos ?? []).some((s) => String(s).trim().toUpperCase() === 'OPERACOES');
}
