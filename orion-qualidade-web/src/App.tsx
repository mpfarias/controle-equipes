...
const DOC_TITLE = 'Órion Qualidade';
...
export default function App() {
  ...
  if (loading) {
    return (
      <div className="app-qualidade">
        <Box sx={{ display: 'flex', justifyContent: 'center', py: 8 }}>
          <CircularProgress sx={{ color: 'var(--qualidade-accent)' }} />
        </Box>
      </div>
    );
  }
  ...
  if (!currentUser) {
    return (
      <div className="app-qualidade app-qualidade--auth">
        {bootstrapError ? (
          <Box sx={{ px: 2, pt: 2, maxWidth: 560, mx: 'auto', flexShrink: 0 }}>
            <Alert severity="warning">{bootstrapError}</Alert>
          </Box>
        ) : null}
        <div className="app-qualidade__auth-shell">
          <LoginQualidadeView onSuccess={handleLoginSuccess} />
        </div>
    ...
    <div className="app-qualidade">
      <Box
        ...
        sx={{
          ...
          borderBottom: `1px solid ${alpha('#38bdf8', 0.22)}`,
          background: `linear-gradient(
            105deg,
            ${alpha('#0c4a6e', 0.97)} 0%,
            ${alpha('#0f172a', 0.98)} 42%,
            ${alpha('#0369a1', 0.32)} 100%
          )`,
          ...
          <Stack direction="row" alignItems="center" spacing={2} sx={{ flex: 1, minWidth: 0 }}>
            <Box
              sx={{
                ...
                height: { xs: 46, sm: 54 },
                borderRadius: 2.5,
                background: `linear-gradient(145deg, ${alpha('#38bdf8', 0.45)} 0%, ${alpha('#0369a1', 0.92)} 50%, ${alpha('#0c4a6e', 1)} 100%)`,
                border: `1px solid ${alpha('#38bdf8', 0.42)}`,
                boxShadow: `${`0 0 0 1px ${alpha('#000', 0.25)} inset`}, 0 10px 32px -14px ${alpha('#38bdf8', 0.45)}`,
                display: 'flex',
                alignItems: 'center',
            ...
            <Box sx={{ minWidth: 0 }}>
              <Stack direction="row" alignItems="center" gap={1} flexWrap="wrap" sx={{ mb: 0.25 }}>
                ...
                <Chip
                  icon={<FactCheck sx={{ fontSize: '16px !important', ml: '4px !important' }} />}
                  ...
                  sx={{
                    ...
                    color: alpha('#e0f2fe', 0.95),
                    border: `1px solid ${alpha('#38bdf8', 0.3)}`,
                    '& .MuiChip-icon': { color: 'var(--qualidade-accent)' },
                  }}
                />
              </Stack>
              <Typography
              ...
              >
                <Box component="span" sx={{ display: { xs: 'none', sm: 'inline' } }}>
                  Registros, acompanhamento e visão consolidada dos indicadores de qualidade.
                </Box>
          ...
          <Stack direction="row" alignItems="center" spacing={1.75} sx={{ flexShrink: 0 }}>
            ...
            <Box
              sx={{
                p: '3px',
                borderRadius: '50%',
                background: `linear-gradient(135deg, ${alpha('#38bdf8', 0.5)}, ${alpha('#0369a1', 0.28)})`,
              }}
            >
              <IconButton
                onClick={(e) => setAvatarMenuAnchor(e.currentTarget)}
                aria-label="Menu do usuário"
                aria-controls={avatarMenuAnchor ? 'user-menu-qualidade' : undefined}
                aria-haspopup="true"
      ...
      {senhaModalOpen && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="alterar-senha-qualidade-titulo"
          onClick={closeSenhaModal}
        >
          <div className="modal" style={{ maxWidth: 440 }} onClick={(e) => e.stopPropagation()}>
            <h3 id="alterar-senha-qualidade-titulo">Alterar senha</h3>
